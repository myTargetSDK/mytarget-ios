...
class VideoPlayerView: UIView
{
	...
	var currentTime: TimeInterval
	{
		get
		{
			guard let player = player else { return _currentTime }
	...
	var volume: Float
	{
		get
		{
			guard let player = player else { return _volume }
	...
	private var playerItemDuration: TimeInterval
	{
		get
		{
			guard let playerItem = playerItem, playerItem.status == .readyToPlay else { return 0.0 }
			guard CMTIME_IS_VALID(playerItem.duration) else { return 0.0 }
...
// MARK: - observers
	...
	{
		guard let object = notification.object as? AVPlayerItem, let playerItem = playerItem, object == playerItem else { return }
		stop()
		delegateOnVideoComplete()
	}
