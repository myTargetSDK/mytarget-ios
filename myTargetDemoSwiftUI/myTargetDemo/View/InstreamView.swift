		...
		ScrollView {
			VStack(spacing: 8) {
				InstreamGrid(header: "Instream parameters", rows: [
					.init(title: "Fullscreen:", value: viewModel.fullscreen),
					.init(title: "Timeout:", value: viewModel.timeout),
					.init(title: "Quality:", value: viewModel.quality),
					.init(title: "Volume:", value: viewModel.volume)
				])
				InstreamGrid(header: "Current ad parameters", rows: [
					.init(title: "Duration:", value: viewModel.duration),
					.init(title: "Allow pause:", value: viewModel.allowPause),
					.init(title: "Position:", value: viewModel.position),
					.init(title: "Allow close:", value: viewModel.allowClose),
					.init(title: "Dimension:", value: viewModel.dimension),
					.init(title: "Close delay:", value: viewModel.closeDelay)
				])
				...
				VStack(spacing: 1) {
					ZStack {
						...
						} else if viewModel.isAdVideoActive, let instreamAd = InstreamAd(playerView: viewModel.adPlayerView, instreamViewController: $viewModel.currentInstreamViewController) {
							instreamAd
								.onDisappear {
