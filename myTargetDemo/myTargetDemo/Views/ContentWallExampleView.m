...
@implementation ContentWallExampleView
{
...
- (instancetype)initWithController:(UIViewController *)controller slotId:(NSUInteger)slotId
{
	...
	if (self)
	{
		...
		[_collectionView registerClass:[BorderCollectionViewCell class] forCellWithReuseIdentifier:kContentWallExampleViewTextCellId];
		[_collectionView registerClass:[BorderCollectionViewCell class] forCellWithReuseIdentifier:kContentWallExampleViewAdCellId];
...
- (void)onLoadWithNativePromoBanner:(MTRGNativePromoBanner *)promoBanner nativeAd:(MTRGNativeAd *)nativeAd
{
	...
	_adView = [MTRGNativeViewsFactory createContentWallViewWithBanner:promoBanner];
	[_adView loadImages];
	[_nativeAd registerView:_adView withController:_controller];

	[_views insertObject:_adView atIndex:kContentWallExampleViewAdIndex];
	[_collectionView reloadData];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (indexPath.row == kContentWallExampleViewAdIndex)
	{
		cell = [collectionView dequeueReusableCellWithReuseIdentifier:kContentWallExampleViewAdCellId forIndexPath:indexPath];
		if (!cell)
	...
	{
		cell = [collectionView dequeueReusableCellWithReuseIdentifier:kContentWallExampleViewTextCellId forIndexPath:indexPath];
		if (!cell)
			cell = [[BorderCollectionViewCell alloc] init];
		[cell.contentView addSubview:view];
	}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
	...
	CGFloat height;
	if ([view isKindOfClass:[MTRGContentWallAdView class]])
	{
		CGFloat padding = 6;
